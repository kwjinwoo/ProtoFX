...
- `docs/status/SUPPORT_MATRIX.md` was framed as a detailed current-state matrix, which is costly to maintain by hand
...
- `docs/status/SUPPORT_MATRIX.md` becomes a representative summary rather than an exhaustive model-by-task matrix.
...
|-------------|----------------|
| Keep `docs/status/SUPPORT_MATRIX.md` as a detailed exhaustive matrix | Too much direct-maintenance cost for a document that is not authoritative in the first place |
